...
class _SetupScreenState extends State<SetupScreen> {
  ...
  Future<void> _login() async {
    ...
    // 1. Check Whitelist (Gatekeeper)
    final isAllowed = await gatekeeper.isUserAllowed(
    ...
    if (!isAllowed) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Access Denied: User not whitelisted")),
      );
      return;
    }

    // 2. Check Child Agent Status (Heartbeat)
